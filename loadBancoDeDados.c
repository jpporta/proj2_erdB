...
// recebe: nome da cidade, array de cidades, inicio do vetor, fim do vetor, meio
// para destravar
int encontraCidade(char *name, tCity *cidades, int inicio, int final, int m) {
  if (final >= 0) {
    ...
    if (strcmp(cidades[meio].name, name) == 0)
      return meio;
    if (strcmp(cidades[meio].name, name) > 0)
      return encontraCidade(name, cidades, inicio, (meio - 1), meio);
    if (strcmp(cidades[meio].name, name) < 0)
      return encontraCidade(name, cidades, (meio + 1), final, meio);
  }
  return -1;
}
  ...
  while (!feof(arq)) {
    ...
    if (c != '!') {
      ...
      for (int i = 0; i < qtdCity; i++) {
        ...
        palavra[counter] = '\0';
        cidades[cityAtual].vizinhos[i] =
            &(cidades[encontraCidade(palavra, cidades, 0, NCIDADES - 1, 0)]);
      }
      for (int i = 0; i < qtdCity; i++) {
        fscanf(arq, "%i", &distancia);
      }
